...
loTimeMax = 1,									//po ilu sekundach ma zmienic na przycisk hi
hiTimeMax = 1,									//po ilu sekundach ma zmienic na przycisk lo
...
clickTime = Math.floor(d.getTime()/1000),		//zmienna do trzymania kiedy ostatni raz byl klikniety przycisk
...
function kliknij(){									//funkcja klikajaca	
	var time = Math.floor(d.getTime()/1000);		//jako czas przyjmuje podloge z czasu/sekunde
	if(buttonLo){									//jezeli przycisk lo jest true to ma wykonac akcje dla lo
		if(time-clickTime<loTimeMax){				//jezeli ostatnie klikniecie bylo mniej niz loMax sekund
		...
		if(time-clickTime<hiTimeMax){				//jezeli licznik jest mniejszy od max
		 	$hiButton.trigger('click');				//to klika w przycisk hi
		}else{										//jezeli jest rowny albo wiekszy 
			clickCount = time;						//to licznik jest zerowany 
